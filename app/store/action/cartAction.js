...
import { ADD_CART, ADD_TOCART, APPLY_COUPAN, APPLY_COUPON, DELETE_CART_PRODUCT, GET_CART, ORDER_HISTORY, UPDATE_CART } from '../../queries/orderQuery';
    ...
    query(GET_CART, { "id": userID })
      .then(async response => {
        if (!isEmpty(response.data.cartbyUser.products)) {
          ...
          dispatch({
            type: CHECK_STORAGE,
            payload: { cartProducts: cartProducts, cartID: response.data.cartbyUser.id },
          });
        } else if (!isEmpty(response.data.cartbyUser.id) && (isEmpty(response.data.cartbyUser.products))) {
          dispatch({
            type: CHECK_STORAGE,
...
export const removeFromCartAction = (payload, userID) => dispatch => {
  ...
  mutation(DELETE_CART_PRODUCT, payload)
    .then(response => {
      if (response) {
        if (!isEmpty(response.data.deleteCartProduct) && response.data.deleteCartProduct.success) {
          dispatch({
            type: CART_FAIL,
          });
          dispatch(checkStorageAction(userID))
    ...
    .catch(error => {
      ...
      dispatch({
        type: ALERT_ERROR,
        payload: response.data.calculateCoupon.message || 'Something went wrong. Please try again later.',
      });
    });
};
export const applyCouponAction = payload => dispatch => {
  ...
  query(APPLY_COUPON, payload)
    .then(response => {
      if (response) {
        if (!isEmpty(response.data.calculateCoupon) && response.data.calculateCoupon.total_coupon > 0) {
          dispatch({
            type: COUPON_APPLIED,
...
export const orderHistoryAction = payload => dispatch => {
  dispatch({
    type: ORDER_LOADING,
  });

  query(ORDER_HISTORY, payload)
    .then(response => {
      if (response) {
        if (!isEmpty(response.data.orderbyUser) && response.data.orderbyUser.message.success) {
          dispatch({
            type: ORDER_SUCCESS,
