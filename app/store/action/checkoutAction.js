...
export const checkoutDetailsAction = (checoutDetailsData,cartId, navigation) => (dispatch) => {
  console.log(checoutDetailsData, 'payload')
  dispatch({
  ...
  mutation(ADD_ORDER, checoutDetailsData)
    .then(async response => {
      if (response) {
        console.log(response.data.addOrder.success, 'response.data.calculateCoupon.total_coupon')
        if (response.data.addOrder.success) {
          dispatch({
            type: REMOVE_ALL_CART_PRODUCT
          })  
          let cartValue =await getValue('cartproducts')
          console.log(!isEmpty(cartValue),'!isEmpty(cartValue)')
          // if(!isEmpty(cartValue)){
