...
export const addCartAction = payload => dispatch => {
  ...
  mutation(ADD_CART, payload)
    .then(async response => {
      if (response.data.addCart.success) {
        dispatch({
          type: RESPONSE_HANDLE,
...
export const removeFromCartAction = (payload, userID) => dispatch => {
  ...
  mutation(DELETE_CART_PRODUCT, payload)
    .then(response => {
      if (response) {
        if (response.data.deleteCartProduct.success) {
          dispatch({
            type: CART_FAIL,
          });
          dispatch(checkStorageAction(userID))
...
export const applyCouponAction = payload => dispatch => {
  ...
  query(APPLY_COUPON, payload)
    .then(response => {
      if (response) {
        console.log(response, 'response.data.calculateCoupon.total_coupon')
        if (response.data.calculateCoupon.total_coupon > 0) {
          dispatch({
            type: COUPON_APPLIED,
...
export const orderHistoryAction = payload => dispatch => {
  ...
  query(ORDER_HISTORY, payload)
    .then(response => {
      if (response) {
        if (response.data.orderbyUser.message.success) {
          dispatch({
            type: ORDER_SUCCESS,
