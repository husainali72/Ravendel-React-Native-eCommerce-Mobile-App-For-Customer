...
const ProductPriceText = ({ Pricing, fontsizesmall, DontshowPercentage, showInMulipleLine, fontColor }) => {
    ...
    return (
        <ProductPriceView fd={showInMulipleLine ? showInMulipleLine : 'row'}>
            {Pricing.sellprice && Pricing.sellprice < Pricing.price ? (
                <>
                    <AText
                        ...
                        heavy>{formatCurrency(Pricing.sellprice, currencyOptions, currencySymbol)}{'  '}
                    </AText>
                </>
            ) : null}
            {/* ===============Has Sale Price============= */}
            <AText
                ...
                large={Pricing.sellprice < Pricing.price ? false : !isEmpty(fontsizesmall) ? !fontsizesmall : true}
                heavy={Pricing.sellprice < Pricing.price ? false : true}
                color={
                    Pricing.sellprice < Pricing.price ? '#7b7b7b' : '#000000'
                }>{formatCurrency(Pricing.price, currencyOptions, currencySymbol)}
            </AText>
            {(isEmpty(DontshowPercentage) || !DontshowPercentage) && (Pricing.sellprice < Pricing.price)
                ?
                <>
                    {/* ===============Has Discount Price============= */}
